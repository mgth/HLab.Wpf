...
namespace HLab.Mvvm.Application.Wpf
{
   ...
   public class MenuService(MainWpfViewModel viewModel) : IMenuService
   {
      public object MainMenu => throw new System.NotImplementedException();
      ...
      public bool RegisterMenu(string path, object header, ICommand command, string iconPath)
      {
         ...
         {
            return RegisterMenu(MenuPath.Get(path), viewModel.Menu.Items, header, command, iconPath);
         }

         var result = false;
         System.Windows.Application.Current.Dispatcher.Invoke(() =>
            result = RegisterMenu(MenuPath.Get(path), viewModel.Menu.Items, header, command, iconPath));
         return result;
      }
