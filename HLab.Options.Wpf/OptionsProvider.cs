...
public class OptionsProviderRegistry(IOptionsService options) : IOptionsProvider
{
   public class Loader(OptionsProviderRegistry optionProvider, IOptionsService options) : Bootloader 
   {
      protected override BootState Load()
      {
         options.AddProvider(optionProvider);
         return base.Load();
      }
   }
